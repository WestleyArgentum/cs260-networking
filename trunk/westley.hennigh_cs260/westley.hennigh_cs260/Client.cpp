...
// the maximum length of a message we will receive
static const unsigned BUFF_SIZE = 1024;

extern HWND output;


Client::Client( std::string server_ip, unsigned port /*void (*callback_)(IMessage)*/ ) : ipAddress(server_ip), remote_port(port), lost_server(false), rdy(0)
{}
...
int Client::Connect(std::string username_)
{
  ...
  ret = recv(clientSocket, buffer, BUFF_SIZE, 0); 
